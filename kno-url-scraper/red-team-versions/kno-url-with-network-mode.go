...
func runNetworkMode(urlStr string, args []string) {
	...
	for i < len(args) {
		...
		case "-o":
			if i+1 >= len(args) {
				fmt.Println("Error: -o requires a filename (network mode).")
				return
			}
			outputFile = args[i+1]
			i += 2
		case "-t":
			if i+1 >= len(args) {
				fmt.Println("[-] Network mode (-n) requires -t <duration> or --live.")
				return
			}
			sec, err := parseDurationToSeconds(args[i+1])
			if err != nil {
				fmt.Printf("[-] Invalid duration for -t: %v\n", err)
				return
			}
			durationSeconds = sec
			i += 2
		...
		case "--search":
			if i+1 >= len(args) || strings.HasPrefix(args[i+1], "-") {
				fmt.Println("Error: --search requires a value like 'mp4' or 'mp4,cdn'")
				return
			}
			raw := args[i+1]
			i += 2
			...
			if len(searchTerms) == 0 {
				fmt.Println("Error: --search requires at least one non-empty term")
				return
			}
		default:
			if tname, ok := flagToNetType[a]; ok {
				selectedTypes[tname] = true
				i++
			} else {
				// Unknown here (e.g. HTML-only flags), ignore; warning was already printed
				i++
			}
		}
	}

	if !liveMode && durationSeconds <= 0 {
		fmt.Println("[-] Network mode (-n) requires either -t <duration> or --live.")
		return
	}

	// If no type filters, capture all
	shouldKeepType := func(t string) bool {
		if len(selectedTypes) == 0 {
			return true
		}
		return selectedTypes[t]
	}

	fmt.Printf("[*] Starting Playwright network capture for %s ...\n", urlStr)
	pw, err := playwright.Run()
	if err != nil {
		fmt.Printf("[-] Failed to start Playwright: %v\n", err)
		fmt.Println("    Make sure playwright-go is installed and browsers are set up.")
		return
	}
	defer func() {
		_ = pw.Stop()
	...
	if err != nil {
		fmt.Printf("[-] Failed to launch Chromium: %v\n", err)
		return
	}
	defer browser.Close()
	...
	if err != nil {
		fmt.Printf("[-] Failed to create browser context: %v\n", err)
		return
	}
	...
	page.On("request", func(request playwright.Request) {
		...
		tname := mapResourceTypeToGroup(rt, reqURL)
		if !shouldKeepType(tname) {
			return
		}
		if !matchesSearch(reqURL, searchTerms) {
			return
		}
		if _, ok := captured[tname]; !ok {
			captured[tname] = map[string]bool{}
		}
		captured[tname][reqURL] = true
		if liveMode {
			fmt.Printf("[%s] %s\n", tname, reqURL)
		}
	})

	_, err = page.Goto(urlStr)
	if err != nil {
		fmt.Printf("[-] Page navigation failed: %v\n", err)
		return
	}

	if liveMode {
		fmt.Println("[*] Live mode: capturing requests. Press Ctrl+C to stop.")
		// Just sleep "forever"; user will Ctrl+C.
		for {
			time.Sleep(5 * time.Second)
		}
	} else {
		fmt.Printf("[*] Capturing network traffic for %d seconds ...\n", durationSeconds)
		time.Sleep(time.Duration(durationSeconds) * time.Second)
	}

	// Build summary
	order := []string{
		"Fetch/XHR", "Doc", "CSS", "JS", "Font", "Img",
		"Media", "Manifest", "Socket", "Wasm", "Other",
	}

	var lines []string
	for _, tname := range order {
		urlSet, ok := captured[tname]
		if !ok || len(urlSet) == 0 {
			continue
		}
		lines = append(lines, tname)
		// sort URLs
		var us []string
		for u := range urlSet {
			us = append(us, u)
	...
	if text == "" {
		fmt.Println("[*] No network requests matched the selected filters.")
		return
	}
	...
	if outputFile != "" {
		if err := os.WriteFile(outputFile, []byte(text+"\n"), 0644); err != nil {
			fmt.Printf("[-] Failed to write network results to %s: %v\n", outputFile, err)
		} else {
			fmt.Printf("[*] Network results written to %s\n", outputFile)
		}
	}
}
